...
public class utility {
    ...
    public static boolean RollDice(int Bounds){
        ...
        if (TypeRoll==0){System.out.println("Бросок на удачу! Успех - выбить число не меньше "+Bounds+" !");
          while (true){ try {input = in.nextInt();} catch (Exception e){System.out.println("Вы ввели хуйню, вводите снова.");continue;}if (input>Bounds){System.out.println("Вы ввели число, превышающее границы ("+Bounds+") !");continue;}break;}
          ...
          if (result>=Bounds){System.out.println("Успех! вы выбросили: "+result+" / "+Bounds);return true;}
            ...
            while (true){ try {input = in.nextInt();} catch (Exception e){System.out.println("Вы ввели хуйню, вводите снова.");continue;}break;}
    ...
    public static boolean FindID(String InputName,ArrayList<CharactersMobs> Mobs){
        boolean Found = false; int inputNumber=0;
        for (CharactersMobs mob : Mobs) {
            try {
                inputNumber = Integer.parseInt(InputName);
            }catch (Exception e){System.out.println("Вы не число ввели");return Found;}

            if (inputNumber == CharactersMobs.GetIdIndex(mob)) {CharactersMobs.PersonA = mob;Found=true;break;}
        }
